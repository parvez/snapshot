...
(function($) {
    ...
    var methods = {
        init : function(opts) {
            ...
            block["period"] = $("<div class='cron-period'>"
            ...
            block["dom"] = $("<div class='cron-block cron-block-dom'>"
            ...
            block["month"] = $("<div class='cron-block cron-block-month'>"
            ...
            block["mins"] = $("<div class='cron-block cron-block-mins'>"
                    + " at <select name='cron-mins'>" + str_opt_mih
                    + "</select> minute(s) past the hour </div>")
                .appendTo(this)
                .data("root", this);
            ...
            block["dow"] = $("<div class='cron-block cron-block-dow'>"
            ...
            block["time"] = $("<div class='cron-block cron-block-time'>"
            ...
            block["controls"] = $("<div class='cron-controls'>&laquo; save "
                    + "<div class='cron-button cron-button-save'></div>"
                    + " </div>")
                .appendTo(this)
                .data("root", this)
                .find("div.cron-button-save")
        ...
        string_value : function() {
          function padTime(m, h, t) {
            ...
            if (h) {
              return Array(Math.max(2 - String(h).length + 1, 0)).join(0) + h + ":" + Array(Math.max(2 - String(m).length + 1, 0)).join(0) + m + _s;
            } else {
              return Array(Math.max(2 - String(m).length + 1, 0)).join(0) + m;
            }
          }
          var c = this;
          var b = c.data("block");
          ...
          switch (selectedPeriod) {
              ...
              case "hour":
                min = b["mins"].find("select").val();
                ev += " at " + padTime(min) + " minute(s) past the hour"
                  break;

              case "day":
                  min  = b["time"].find("select.cron-time-min").val();
                  hour = b["time"].find("select.cron-time-hour").val();
                  ev += " at " + padTime(min, hour, "12h")
                  break;
              ...
              case "week":
                  ...
                  hour = b["time"].find("select.cron-time-hour").val();
                  dow  =  b["dow"].find("select").val();
                  ev += " on " + days[dow] + " at " + padTime(min, hour, "12h")
                  break;
              ...
              case "month":
                  ...
                  hour = b["time"].find("select.cron-time-hour").val();
                  day  = b["dom"].find("select").val();
                  ev += " on the " + dom_days[day-1] + " at " + padTime(min, hour, "12h")
                  break;
              ...
              case "year":
                  ...
                  day  = b["dom"].find("select").val();
                  month = b["month"].find("select").val();
                  ev += " on the " + dom_days[day-1] + " of " + months[month-1] + " at " + padTime(min, hour, "12h")
                  break;
    ...
    var event_handlers = {
        periodChanged : function() {
            ...
            root.find("div.cron-block").hide(); // first, hide all blocks
